# High-Performance AI Email Classifier
...
A high-throughput email classification engine routing inbound email messages into structured mailbox categories and isolating phishing and spam threats.

---

## Key Features

- **Multi-label**: classification (Support, Sales, Invoicing, Newsletter, Spam, Phishing)
- **Urgency**: level scoring and VIP sender flag detection
- **Phishing**: heuristic scan detecting spoofed domains and urgent payment demands
- **Mailbox**: folder routing recommendation
...
```mermaid
flowchart TD
    Email([Inbound Email]) --> Parser[Header & Body Parser]
    Parser --> ThreatScan{Phishing / Spam Check}
    ThreatScan -->|Threat| Quarantine[Quarantine Folder]
    ThreatScan -->|Safe| Classifier[Category Classifier]
    Classifier --> Route[Assign Mailbox Tag & Priority]
```
...
├── tests/
│   ├── __init__.py
│   └── test_email_classifier.py   # Automated test suite
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/ai-email-classifier.git
...
|---|---|---|
| `POST` | `/classify-email` | Classify email body and headers into operational categories |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/classify-email -H "Content-Type: application/json" -d '{"sender": "billing@cloudhost.com", "subject": "Monthly Invoice #8812", "body": "Your monthly cloud hosting invoice is ready for download."}'
```
...
```bash
python -m unittest tests/test_email_classifier.py
```
